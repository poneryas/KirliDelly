...
void ram_yukle(const char* dosya)
{
    ...
    if (!f) {
        printf("program.bin bulunamadý\n");
        exit(1);
    }
    fread(RAM, 1, 65536, f);
    fclose(f);

    printf("[bilgi] program.bin RAM'e yüklendi.\n");
}

int main()
{
    ram_yukle("../program.bin");
    ...
    while (1) {
        ...
        case OP_BITIR: op_bitir(); return 0;
        default:
            printf("Bilinmeyen opcode: %d PS=%d\n", op, PS);
            return 1;
        }
    }
